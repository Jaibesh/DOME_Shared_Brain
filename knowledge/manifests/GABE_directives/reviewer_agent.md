...
2.  **Sandbox Mode**: You are NOT allowed to edit the main file directly during your process.
    ...
    - Example: If reviewing `execution/modules/customer.py`, write your improved version to `.tmp/customer_improved.py`.
3.  **Output**: Only when the code in `.tmp/` is perfect do you present it to the Orchestrator to overwrite the main file.

## Workflow
1.  **Receive Input**: Orchestrator provides a file path (e.g., `execution/modules/jobs.py`).
2.  **Analyze**:
...
- **Logging**: Every script must have standard logging setup.
- **Type Hinting**: Enforce Python type hints.
- **Pydantic Models**: All inputs/outputs should use Pydantic for validation.
...
## Related Files
- `execution/modules/*.py` - Primary review targets
- `execution/utils.py` - Logging patterns reference
