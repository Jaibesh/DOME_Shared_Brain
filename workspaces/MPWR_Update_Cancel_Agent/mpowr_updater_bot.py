"""
mpowr_updater_bot.py — MPOWR Update & Cancel Playwright Automation Engine

Specialized bot for rescheduling and canceling existing MPOWR reservations.
Does NOT contain reservation creation logic — that lives in the MPWR_Reservation_Agent.
...
class MpowrUpdaterBot:
    ...
    def _handle_date_selection(self, customer: dict):
        ...
        try:
            ...
            if not cal_found:
                ...
                # 'exact=True' ensures we match solely "Start Date", avoiding substring matches.
                # In Playwright, get_by_label automatically finds the input tied to <label for="id">
                date_input = self._page.get_by_label("Start Date", exact=True).first
                try:
        ...
        try:
            ...
            if assure_card.is_visible():
                ...
                if option.is_visible():
                    ...
                    print(f"  ⚠️ Insurance option '{insurance_label}' not visible in modal")
                    self._page.keyboard.press("Escape")
            else:
    ...
    # -----------------------------------------------------------------------
    # Reservation Management (Cancellation & Payments)
    # -----------------------------------------------------------------------

    def cancel_reservation(self, mpwr_id: str) -> bool:
        """Cancel an existing MPOWR reservation securely."""
        try:
            log.info(f"[Creator Bot] Canceling MPWR {mpwr_id}...")
            # MED-4 FIX: Track if WE opened the browser
            ...
            canceled_badge = self._page.locator("span, div").filter(has_text=re.compile(r"^Canceled$|^Cancelled$", re.IGNORECASE)).first
            if canceled_badge.is_visible():
                log.info(f"  [Creator Bot] Reservation {mpwr_id} is already marked as Canceled in MPOWR. Gracefully exiting.")
            ...
            for attempt in range(3):
                ...
                if actions_visible:
                    ...
                    log.warning(f"  [Cancel] Actions button not visible (attempt {attempt + 1}). Waiting...")
                    time.sleep(3)
            
            if not actions_clicked:
                raise ValueError("Could not find 'Actions' button on Reservation Details page.")

            # 2. Click Cancel Reservation
            cancel_btn = self._page.get_by_text("Cancel Reservation", exact=False).first
            ...
            log.error(f"Failed to cancel {mpwr_id}: {e}")
            self._screenshot(f"error_cancel_{mpwr_id}")
            if self._owns_browser:
                self._close_browser()
            return False
    ...
    def update_reservation(self, mpwr_id: str, new_payload: dict) -> bool:
        """Navigates to the Edit/Reschedule page for an existing MPOWR reservation,
        resets vehicles securely, and applies the new dates and vehicles.
        
        Args:
            mpwr_id: MPOWR confirmation ID (e.g., "CO-MP7-AA4")
            new_payload: Dict with keys: activity, start_date_obj, end_date_obj,
                         time_raw, vehicle, vehicle_qty, subtotal
        """
        try:
            log.info(f"[Creator Bot] Launching Edit/Reschedule for MPWR {mpwr_id}...")
            # MED-4 FIX: Track if WE opened the browser so we know if we should close it
            self._owns_browser = False
            if not self._browser or not self._is_session_alive():
                self._start_browser()
                self._owns_browser = True
            ...
            # 1. Click 'Reschedule' via Actions menu to hit the identical Creation-Edit UI
            #    MPOWR's Headless UI dropdown can be slow to render — retry up to 3 times
            reschedule_clicked = False
            reschedule_variants = ["Reschedule", "Edit Order", "Modify"]
            ...
            for attempt in range(3):
                ...
                if actions_visible:
                    ...
                    # Try each known button text variant
                    for variant in reschedule_variants:
                        btn = self._page.get_by_text(variant, exact=False).first
                        try:
                            if btn.is_visible(timeout=1500):
                                btn.click()
                                reschedule_clicked = True
                                log.info(f"  ✅ Clicked '{variant}' (attempt {attempt + 1})")
                    ...
                    # Dropdown opened but no matching item — close it and retry
                    self._page.keyboard.press("Escape")
                    time.sleep(1)
            ...
            if not reschedule_clicked:
                # Last resort: take a screenshot of what the dropdown actually shows
                self._screenshot(f"debug_actions_menu_{mpwr_id}")
                raise ValueError("Could not find 'Reschedule' button in Actions dropdown after 3 attempts.")

            # 2. Re-apply the Listing/Activity type (inline — same logic as create_reservation)
            target_activity = new_payload["activity"]
            ...
            try:
                ...
                target_variants = [target_activity]
                if "'" in target_activity:
                    target_variants.append(target_activity.replace("'", "\u2019"))
                if "\u2019" in target_activity:
                    target_variants.append(target_activity.replace("\u2019", "'"))
                ...
                for scroll_attempt in range(6):
                    for tv in target_variants:
                        ...
                        for ci in range(card_count):
                            ...
                            is_blocked = any(bl in card_text for bl in self.SPECIAL_EVENT_BLOCKLIST)
                            if is_blocked:
                                log.info(f"  ⏭️ Skipping blocked listing: {card_text[:60]}")
                                continue
                            ...
                            candidate.click()
                            activity_selected = True
                            log.info(f"  ✅ Selected listing: {tv} (scroll {scroll_attempt})")
                            time.sleep(2)
                            break
                ...
                if not activity_selected:
                    for tv in target_variants:
                        ...
                        if card_fb.count() > 0:
                            ...
                            is_blocked = any(bl in fb_text for bl in self.SPECIAL_EVENT_BLOCKLIST)
                            if is_blocked:
                                log.info(f"  ⏭️ Skipping blocked fallback: {fb_text[:60]}")
                                continue
                            card_fb.scroll_into_view_if_needed()
                            card_fb.click()
                            activity_selected = True
                            log.info(f"  ✅ Selected listing (fuzzy): {tv}")
                            time.sleep(2)
                            break
            except Exception as e:
            ...
            # 3. Re-apply the Dates — build a compatibility dict for _handle_date_selection
            start_dt = new_payload.get("start_date_obj")
            end_dt = new_payload.get("end_date_obj")
            is_multiday = start_dt.date() != end_dt.date() if start_dt and end_dt else False

            date_compat = {
                "activity_date": start_dt.strftime("%m/%d/%Y") if start_dt else "",
                "mpowr_activity": "Multi-Day Adventure Rental" if is_multiday else new_payload["activity"],
            }
            # For multi-day, the end date is handled by _handle_date_selection reading normalized_date
            if is_multiday and end_dt:
                date_compat["end_date"] = end_dt.strftime("%m/%d/%Y")

            self._handle_date_selection(date_compat)
            time.sleep(2)

            # 4. Re-apply the Time
            if 'time_raw' in new_payload and new_payload['time_raw']:
                self._handle_time_selection(new_payload['time_raw'])
            else:
                self._handle_time_selection("8:00 AM")
            ...
            # 6. Apply new specific Vehicle constraints
            if "vehicles" in new_payload and new_payload["vehicles"]:
                for v in new_payload["vehicles"]:
                    self._handle_vehicle_selection(v["model"], str(v["qty"]))
            elif "vehicle" in new_payload:
                v_name = new_payload["vehicle"]
                v_qty = str(new_payload.get("vehicle_qty", "1"))
                self._handle_vehicle_selection(v_name, v_qty)

            # 6.5. Apply Guide Addons to sync changed quantities
            if "guide_addons" in new_payload and new_payload["guide_addons"]:
                self._handle_guide_addons(new_payload["guide_addons"])

            # CRIT-2 FIX: Re-select insurance (Reschedule page may clear this)
            try:
                from shared.tripworks_mapper import InsuranceSelector
                insurance_label = InsuranceSelector.get_insurance_selection("rental", "free")
                self._handle_insurance_selection(insurance_label)
            except Exception as ins_err:
                log.warning(f"  ⚠️ Insurance re-selection during update (non-fatal): {ins_err}")

            # CRIT-2 FIX: Re-check mandatory checkboxes (may be required after reschedule)
            try:
                self._handle_additional_questions()
            except Exception as aq_err:
                log.warning(f"  ⚠️ Additional questions during update (non-fatal): {aq_err}")

            # 7. Submit "Update Reservation"
            submit_btn = self._page.get_by_role("button", name="Update Reservation").first
            if self.dry_run:
                log.info(f"DRY RUN: Would have effectively Rescheduled {mpwr_id}")
            else:
                submit_btn.click()
                time.sleep(3)
                log.info(f"✅ Successfully Rescheduled/Updated {mpwr_id} in MPOWR")

            # MED-4 FIX: Only close browser if we opened it ourselves
            if self._owns_browser:
                self._close_browser()
            return True

        except Exception as e:
            log.error(f"Failed to update {mpwr_id}: {e}")
            self._screenshot(f"error_update_{mpwr_id}")
            if self._owns_browser:
                self._close_browser()
            return False
