...
class SlackNotifier:
    ...
    def send_reservation_success(
        ...
        blocks.append({
            ...
            "text": {
                "type": "mrkdwn",
                "text": f"*Reservation Link:*\nhttps://mpwr-hq.poladv.com/orders/{mpowr_conf_id}"
            }
        })
        ...
        fallback_text = (
            ...
            f"Link: https://mpwr-hq.poladv.com/orders/{mpowr_conf_id}\n"
    ...
    def send_duplicate_alert(
        ...
        blocks = [
            ...
            },
            {
                ...
                "text": {
                    "type": "mrkdwn",
                    "text": f"*Reservation Link:*\nhttps://mpwr-hq.poladv.com/orders/{existing_mpowr_id}"
                }
            },
            {"type": "divider"},
        ]
        ...
        fallback_text = (
            ...
            f"Link: https://mpwr-hq.poladv.com/orders/{existing_mpowr_id}"
        )

        self._send_message(blocks, fallback_text)

    def send_minor_update_alert(
        self,
        customer_name: str,
        tw_confirmation: str,
        mpowr_id: str,
        waivers_complete: int,
        waivers_expected: int,
    ):
        """
        Notifies that an update webhook was received but core details haven't changed.
        Primarily used for Waiver signings or minor TripWorks metadata changes.
        """
        blocks = [
            {
                "type": "header",
                "text": {
                    "type": "plain_text",
                    "text": "📝 Minor Update / Waiver Signed",
                    "emoji": True,
                }
            },
            {
                ...
                "text": {
                    "type": "mrkdwn",
                    "text": f"TripWorks sent an update for `{tw_confirmation}` (MPOWR `#{mpowr_id}`), but core reservation details (Date, Time, Price) haven't changed.\n*MPOWR reschedule skipped.*"
                }
            },
            {
                "type": "section",
        ...
        fallback_text = (
            f"📝 Minor Update / Waiver Signed: {customer_name} (TW: {tw_confirmation})\n"
            f"Waivers: {waivers_complete}/{waivers_expected}. MPOWR #{mpowr_id} reschedule skipped."
        )
    ...
    def _upload_file(self, channel: str, file_path: str, comment: str = ""):
        ...
        Uses the modern 2-step upload flow required since Nov 2025:
        1. files.getUploadURLExternal → get a temporary upload URL + file_id
        ...
        try:
            ...
            data1 = resp1.json()
            if not data1.get("ok"):
                print(f"[Slack] getUploadURLExternal failed: {data1.get('error')}")
                return

            upload_url = data1["upload_url"]
            file_id = data1["file_id"]

            # Step 2: Upload file bytes to the temporary URL
            ...
            if resp2.status_code != 200:
                print(f"[Slack] File upload PUT failed: {resp2.status_code}")
                return

            # Step 3: Finalize and share in channel
            resp3 = http_requests.post(
                ...
                json={
                    "files": [{"id": file_id, "title": f"📸 {comment}"}],
                    "channel_id": channel,
            ...
            data3 = resp3.json()
            if data3.get("ok"):
                print(f"[Slack] Screenshot uploaded: {filename}")
            else:
                print(f"[Slack] completeUploadExternal failed: {data3.get('error')}")
    ...
    def send_message(self, text: str, screenshot_path: str | None = None):
        ...
        Used by webhook_processor.py for operational notifications
        (cancellations, payments, skips, errors).
        """
        self._send_message(None, text, screenshot_path)


# Module-level singleton so other modules can do:
